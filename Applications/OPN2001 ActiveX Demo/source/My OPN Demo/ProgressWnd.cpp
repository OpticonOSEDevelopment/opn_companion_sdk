...
const UINT CProgressWnd::TIMER_ID = 25;
/////////////////////////////////////////////////////////////////////////////
// CProgressWnd
...
//    m_CancelButton.SetFont(&m_font, TRUE);
    ...
    SetWindowSize(m_nNumTextLines, 200);
...
//                       TextRect.right, TextRect.bottom+nMargin + CancelSize.cy);
    ...
    ProgressRect.SetRect(TextRect.left, TextRect.bottom + nMargin + EdgeSize.cy, 
                         TextRect.right, TextRect.bottom+nMargin + CancelSize.cy /*- EdgeSize.cy*/);


    // Resize the main window to fit the controls
    CSize ClientSize(nMargin + TextRect.Width() + nMargin,
                     nMargin + TextRect.Height() + nMargin + /*CancelRect.Height()*/ProgressRect.Height() + nMargin);
...
void CProgressWnd::SetText(LPCTSTR fmt, ...)
{
    ...
    va_start(args, fmt);
     vswprintf_s(buffer, 511,  fmt, args);
    va_end(args);
...
void CProgressWnd::SetAutoHide( UINT nHideTime )
{
	if( !::IsWindow( m_hWnd )) 
        return;
	//
	// Set or reset the autohide timer 
	//
	SetTimer( TIMER_ID, nHideTime, NULL );
